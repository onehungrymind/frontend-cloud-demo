...
export class WidgetsEffects {
  @Effect() effect$ = this.actions$.pipe(ofType(WidgetsActionTypes.WidgetsAction));
  ...
  loadWidgets$ = this.dataPersistence.fetch(WidgetsActionTypes.LoadWidgets, {
    run: (action: LoadWidgets, state: WidgetsState) => {
      return this.widgetsService
        .all()
        .pipe(map((res: Widget[]) => new WidgetsLoaded(res)));
    },
  ...
  @Effect()
  addWidget$ = this.dataPersistence.pessimisticUpdate(
    WidgetsActionTypes.AddWidget,
    {
      run: (action: AddWidget, state: WidgetsState) => {
        return this.widgetsService
          .create(action.payload)
          .pipe(map((res: Widget) => new WidgetAdded(res)));
      },
  ...
  @Effect()
  updateWidget$ = this.dataPersistence.pessimisticUpdate(
    WidgetsActionTypes.UpdateWidget,
    {
      run: (action: UpdateWidget, state: WidgetsState) => {
        return this.widgetsService
          .update(action.payload)
          .pipe(map((res: Widget) => new WidgetUpdated(res)));
      },
  ...
  @Effect()
  deleteWidget$ = this.dataPersistence.pessimisticUpdate(
    WidgetsActionTypes.DeleteWidget,
    {
      run: (action: DeleteWidget, state: WidgetsState) => {
        return this.widgetsService
          .delete(action.payload)
          .pipe(map(_ => new WidgetDeleted(action.payload)));
      },
